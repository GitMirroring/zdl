...
function get_item_conf {	
    awk "{match(\$0,/^\s*$1=\"*([^\"]+)\"*$/,pattern); if (pattern[1]) print pattern[1]}" "$file_conf" |
	tail -n1 ## ultimo valore se nel file di configurazione sono presenti valori diversi per una stessa chiave
}
