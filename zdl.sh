...
function update_updater {
	...
	then
		...
		res=($(yad --title="$(gettext "Updating")"                                                                  \
			   --image="$IMAGE2"                                                                                \
			   --text="$(eval_gettext "\$TEXT\n\nA new update is available, choose what to do:\n\n")"           \
			   --form                                                                                           \
			   --center --on-top                                                                                \
			   --separator=' '                                                                                  \
	...
	init_msg+="$(eval_gettext "\${BGreen}\$PROG is already the latest version\n")"
    fi
}
...
then
	...
	then
		...
		print_c 4 "$(eval_gettext "You can control it with: \${Color_Off}\$prog -i \"\$PWD\"\n")"
...
then
	...
	then
		...
		then
		    msg_server="$(eval_gettext "New socket started at the port \$socket_port")"
	    ...
	    then
		msg_server="$(eval_gettext "Socket already in use at the port \$socket_port (you can specify a new port with: -s PORTA or --socket=PORTA)")"
		init_msg+="$(sprint_c 3 "$msg_server\n")"
		echo 1 >"$path_server"/zdl-sockets-exit
		
	    else
		msg_server="$(eval_gettext "Socket not started at the port \$socket_port")"
		init_msg+="$(sprint_c 3 "$msg_server\n")"
...
then
	...
	then
	    ...
	    [[ "${args[i]}" =~ ^\-\-multi ]] &&
		init_msg+="$(eval_gettext "\${BRed}Deprecated option: '--multi = [NUMBER]' is replaced by '--max-downloads = [NUMBER]'. The short option remains unchanged: '-m [NUMERO]'\n")"
...
then
	...
	then
		...
		if [ -e "$print_out" ] 
		then
		    init_msg+=$(sprint_c 3 "$(eval_gettext "A file already exists with the same name as \${print_out}\n"))")
...
####		check_dler_forcing
		...
		then
			...
			local FILEURL=$(eval_gettext "File URL")
...
then
    print_c 0 ""
    print_c 3 "$(gettext "\nIn %s there is a file that contains a list of operations of %s terminated without success.")" "$PWD" "$PROG"
