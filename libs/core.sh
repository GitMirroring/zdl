...
function get_instance_paths {
    awk 'BEGINFILE{if (ERRNO != "") nextfile} /[\/\0]{1}zdl\0/{ split($0, cmdline, "\0"); for(i=0; i<length(cmdline); i++){if (system("test -d \""cmdline[i]"\"")==0){newdir = cmdline[i]; match(FILENAME, /[0-9]+/, matched); c = "cat /proc/" matched[0] "/environ"; c | getline environ; close(c); match(environ,/PWD=[^\0]+/,pwd); if(!(pwd[0] in paths)){paths[pwd[0]]; base = substr(pwd[0],5); if (system("test -d " base "/" newdir)==0){ print system("realpath " base "/" newdir) } else { print base }} }}}' /proc/[0-9]*/cmdline
}

function get_paths_json {
    declare -n ref=$1
    ref=$(awk 'BEGIN{json="["} BEGINFILE{if (ERRNO != "") nextfile} /[\/\0]{1}zdl\0/ { split($0, cmdline, "\0"); for(i=0; i<length(cmdline); i++){if (system("test -d \""cmdline[i]"\"")==0){newdir = cmdline[i]; match(FILENAME, /[0-9]+/, matched); c = "cat /proc/" matched[0] "/environ"; c | getline environ; close(c); match( environ, /PWD=[^\0]+/, pwd); if (!(pwd[0] in paths) && (pwd[0] != "")){ paths[pwd[0]]; base = substr(pwd[0],5); if (system("test -d " base "/" newdir)==0){ dir = system("realpath " base "/" newdir) } else { dir = base }; if (json == "[") { json = json "\"" dir "\"" } else { json = json ",\"" dir "\"" }}}}} END{ print json "]" }' /proc/[0-9]*/cmdline)
}
